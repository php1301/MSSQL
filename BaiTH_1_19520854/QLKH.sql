...
SELECT SP.MASP, TENSP
FROM SANPHAM SP, HOADON HD, CTHD
WHERE  SP.MASP = CTHD.MASP AND CTHD.SOHD = HD.SOHD AND YEAR(HD.NGHD)= 2006 AND SP.NUOCSX='TRUNGQUOC'
...
GO
CREATE TRIGGER TRG11_INSERT_UPDATE_HD ON HOADON 
FOR INSERT, UPDATE
AS 
BEGIN
    DECLARE @NGHD SMALLDATETIME, @MAKH CHAR(4), @NGDK SMALLDATETIME
    -- loop và chọn record vào variable
    SELECT @NGHD = NGHD, @MAKH = MAKH
    FROM INSERTED
    SELECT @NGDK = NGDK 
    FROM KHACHHANG
    -- Loop tiếp và chọn ra
    WHERE MAKH = @MAKH 
    IF(@NGHD < @NGDK)
    BEGIN
...
-- INSERT/UPDATE HOADON
GO
CREATE TRIGGER TRG11_UPDATE_KHACHHANG ON KHACHHANG
FOR UPDATE 
AS 
BEGIN 
    DECLARE @NGDK SMALLDATETIME, @MAKH CHAR(4), @NGHD SMALLDATETIME 
            ...
            IF(@NGHD <= @NGDK)
            BEGIN
                PRINT 'ERROR NGDK PHAI LON HON NGHD LAP'
                ROLLBACK TRANSACTION
            END
            ELSE
            BEGIN
                PRINT 'CAP NHAT KHACHHANG THANH CONG' + ' ' + @MAKH
            END
END

INSERT INTO KHACHHANG(MAKH,HOTEN,DCHI,SODT,NGSINH,NGDK,DOANHSO) VALUES ('KH73','NGUYEN VAN A','731TRAN HUNG DAO,Q5,THHCM','08823451','22/12/2020','22/12/2020',13060000)
INSERT INTO HOADON(SOHD,NGHD,MAKH,MANV,TRIGIA) VALUES (1073,'15/09/2020','KH73','NV02', 840000)
INSERT INTO HOADON(SOHD,NGHD,MAKH,MANV,TRIGIA) VALUES (1074,'16/10/2020','KH73','NV03', 840000)
INSERT INTO HOADON(SOHD,NGHD,MAKH,MANV,TRIGIA) VALUES (1075,'10/08/2020','KH73','NV04', 840000)
